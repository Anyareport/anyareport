...
export default function ResponderAlertsPage() {
  ...
  const alerts = reports.filter((report) => ['verified', 'en_route', 'on_scene'].includes(report.status));

  return (
    <Space direction="vertical" size={16} style={{ width: '100%' }}>
      <Card className="soft-card page-hero">
        <Title level={2} style={{ color: '#fff', marginTop: 0 }}>Alerts</Title>
        ...
        {alerts.map((report) => (
          <Col xs={24} md={12} lg={8} key={report._id}>
            <Card
              ...
              actions={[
                <Link key="open" to={`/responder/incidents/${report._id}`}>Open incident</Link>,
              ]}
            >
              <Space direction="vertical" size={8} style={{ width: '100%' }}>
                <Text>{report.description}</Text>
                <Tag color={report.category === 'Emergency Situations' ? 'red' : 'blue'}>
                  {report.category === 'Emergency Situations' ? 'Urgent response' : 'Verified alert'}
                </Tag>
